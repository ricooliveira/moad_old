nsga2.altered = function (fn, varNo, objDim, lowerBounds = rep(-Inf, varNo), 
          upperBounds = rep(Inf, varNo), popSize = 10, tourSize = 2, 
          generations = 20, cprob = 0.7, XoverDistIdx = 5, mprob = 0.2, 
          MuDistIdx = 10, initial.element.1, initial.element.2) 
{
  cat("********** R based Nondominated Sorting Genetic Algorithm II *********")
  cat("\n")
  cat("initializing the population")
  cat("\n")
  parent <- t(sapply(1:(popSize-2), function(u) array(runif(length(lowerBounds), # changes in the initial population
                                                        lowerBounds, upperBounds))))
  parent = rbind(initial.element.1,initial.element.2,parent)
